  ...
  return (
    <div style={{ display: 'flex', height: '100%' }}>
      <Sidebar backgroundColor="#d1cfff" >
       <Menu transitionDuration={500} menuItemStyles={{
      ...
      >
          <SubMenu label="Mode Itinéraire" backgroundColor="#d1cfff" onClick={() => setMode("itinerary")}>
          {mode === "itinerary" && (
        ...
        <SubMenu backgroundColor="#d1cfff" label="Pose et dépose d'objet" onClick={() => setMode("addIcon")}>
        {mode === "addIcon" && (
          <div  >
            {/* Choix Icones */}
            <label style={{marginLeft: '10px'}}>Sélectionner une icône :</label>
            <div style={{ fontfamily:" Arial Narrow, sansserif", display: "flex", gap: "10px", marginTop: "5px" , marginLeft: '10px'}}>
              {iconImages.map((icon) => (
                <img
                  key={icon}
                  src={`image/${icon}.png`}
            ...
            {/* Nombre d'objets */}
            <p style={{marginLeft: '10px'}}>Nombre de barrière : {count.barrier}</p>
            <p style={{marginLeft: '10px'}}>Nombre de policecar : {count.policecar}</p>
            <p style={{marginLeft: '10px'}}>Nombre de policeman : {count.policeman}</p>
          </div>
        )}
        </SubMenu>
      ...
      <span className="modeJour">{nightMode ? "Mode Nuit" : "MODE JOUR"}</span>
    </div>
